...
class StockOutDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.quantity_edit = create_line_edit("请输入出库数量")
        ...
        ok_btn = create_button("确认出库", "#F44336", "#D32F2F")
    ...
    def handle_ok(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "错误", "出库数量必须是数字")
            return

        if quantity <= 0:
            QMessageBox.warning(self, "错误", "出库数量必须大于0")
            return
        ...
        try:
            StockService.stock_out(product["id"], quantity, remark, self.user["id"])
            QMessageBox.information(
                self, "出库成功", f"成功出库 {quantity} {product['unit']}"
            )
            self.accept()
