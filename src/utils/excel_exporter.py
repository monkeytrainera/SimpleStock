...
class ExcelExporter:
    ...
    def export_ledger(records, file_path):
        ...
        for row, record in enumerate(records, 2):
            ...
            ws.cell(row=row, column=3, value=record["product_name"]).border = (
                thin_border
            )
            qty = (
                f"+{record['quantity']}"
                if record["type"] == "入库"
                else f"-{record['quantity']}"
            )
            ws.cell(row=row, column=4, value=qty).border = thin_border
            ws.cell(row=row, column=5, value=record.get("remark", "")).border = (
                thin_border
            )
            ws.cell(row=row, column=6, value=record["operator_name"]).border = (
                thin_border
            )
